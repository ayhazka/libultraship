...
static void gfx_sp_tri1(uint8_t vtx1_idx, uint8_t vtx2_idx, uint8_t vtx3_idx, bool is_rect) {
    ...
    for (int i = 0; i < 3; i++) {
        ...
        for (int t = 0; t < 2; t++) {
            ...
            if (!is_rect && ((g_rdp.other_mode_h & (1U << G_MDSFT_TEXTPERSP)) == G_TP_NONE)) {
                u -= g_rdp.texture_tile[g_rdp.first_tile_index + t].uls / 2.0f;
                v -= g_rdp.texture_tile[g_rdp.first_tile_index + t].ult / 2.0f;
            } else {
        ...
        if (alpha_threshold) {
            buf_vbo[buf_vbo_len++] = g_rdp.blend_color.a / 255.0f; // For now only alpha is used here
        }
